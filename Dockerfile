...
# 각 원소는 띄어쓰기 기준
ENTRYPOINT ["java", "-jar", "-Dserver.port=${SERVER_PORT}", "-Dgrpc.server.port=${GRPC_PORT}", "-Dspring.datasource.url=${DATASOURCE_URL}", "-Dspring.datasource.username=${DATASOURCE_USERNAME}", "-Dspring.datasource.password=${DATASOURCE_PASSWORD}", "-Dspring.jpa.hibernate.ddl-auto=${JPA_HIBERNATE_DDL_AUTO}","-Dminio.endpoint=${MINIO_ENDPOINT}", "-Dminio.accessKey=${MINIO_ACCESS_USER}","-Dminio.secretKey=${MINIO_SECRET_PASSWORD}","-Dminio.bucketName=${MINIO_BUCKET_NAME}", "/app.jar"]
